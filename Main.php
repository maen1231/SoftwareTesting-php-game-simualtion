...
class AdventureController {
    ...
    public function __construct(CharacterModel $character, array $challenge_database) {
        $this->challenge_database = $challenge_database;
    ...
    public function add_challenge(ChallengeModel $challenge) {
        if(count($this->challenge_database) === 5) {
            return;
        }
        
        if($challenge->get_id() === NULL) {
        ...
        if(count($challenge->get_character_advantage_options()) < 2) {
            return;
        }
        
        if(count($challenge->get_challenge_advantage_options()) < 2) {
            return;
        }
    ...
    public function remove_challenge(?string $id) {
        if(count($this->challenge_database) <= 0) {
            throw new InvalidArgumentException("Database is empty");
        }
        ...
        if(!array_key_exists($id, $this->challenge_database)) {
    ...
    public function validate_adventure() : bool {
        ...
        foreach($this->challenge_database as $challenge) {
            if($challenge->get_failure_next() == "_end" || $challenge->get_succeed_next() == "_end") {
                $found_end = true;
            }
            
            if(count($this->character->get_attributes()) === 0) {
                return false;
            }
            
            if(!array_key_exists($challenge->get_test_attribute(), $this->character->get_attributes())) {
                return false;
            }
        }
        ...
        return $this->findPath(
            $this->challenge_database["_start"],
            array(
                "_start"
            )
        );
    ...
    /*
        $current_list is used to avoid circular paths
    */
    private function findPath(ChallengeModel $challenge, array $current_list) : bool {
        if($challenge->get_failure_next() == "_end" || $challenge->get_succeed_next() == "_end") {
            return true;
        }
        
        $next_succeed = $challenge->get_succeed_next();
        
        if(!in_array($next_succeed, $current_list) && array_key_exists($next_succeed, $this->challenge_database)) {
            array_push($current_list, $next_succeed);
            $found = $this->findPath(
                $this->challenge_database[$next_succeed], 
                $current_list
            );
            ...
            unset($current_list[$next_succeed]);
        }
        
        $next_failure = $challenge->get_failure_next();
        
        if(!in_array($next_failure, $current_list) && array_key_exists($next_failure, $this->challenge_database)) {
            array_push($current_list, $next_failure);
            $found = $this->findPath(
                $this->challenge_database[$next_failure], 
                $current_list
            );
            ...
            unset($current_list[$next_failure]);
        }
        
        return false;
    }
    
    public function navigate_adventure(BinaryRandomOracle $bro) : array {
        $res = array();
        
        $current_adventure = $this->challenge_database["_start"];
        
        $handleFailure = function($ca) {
            $bp = $this->character->get_brownie_points();
            
            if($bp < 3) {
                $this->character->decrement_attribute($ca->get_test_attribute());
                return false;
            }
            
            $this->character->decrement_brownie_points(3);
            return true;
        };
        
        while(true) {
            $run_challenge_res = $current_adventure->run_challenge($bro, $this->character);
            
            for($i = 1; $i < count($run_challenge_res["output"]); $i++) {
                $res[] = $run_challenge_res["output"][$i];
            }
            
            if($run_challenge_res["success"] && $current_adventure->get_succeed_next() == "_end") {
                $this->character->increment_brownie_points(1);
                break;
            }
            
            if(!$run_challenge_res["success"] && $current_adventure->get_failure_next() == "_end"){
                $handleFailure($current_adventure);
                break;
            }
            
            if($run_challenge_res["success"]) {
                $this->character->increment_brownie_points(1);
                $current_adventure = $this->challenge_database[$current_adventure->get_succeed_next()];
            } 
            else {
                if($handleFailure($current_adventure)) {
                    $current_adventure = $this->challenge_database[$current_adventure->get_succeed_next()];
                } else {
                    $current_adventure = $this->challenge_database[$current_adventure->get_failure_next()];
                }
            }
